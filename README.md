...
Each run saves the selected repositories' original remote URLs under `%LOCALAPPDATA%\KiroRaceCo\LocalGitMigrationTool\backups` and provides a **Restore original remotes** button in the final summary. Full diagnostic logs are saved to `\\uk-files-01\dropbox\08_IT\_Software\LocalGitMigrationTool\Log` as `<Windows-username>_<timestamp>_<successful-repository-count>-repos.log`. At startup, the tool requires access to this folder; if it cannot reach it, it asks the user to select their Silverstone drive's `08_IT` folder.
